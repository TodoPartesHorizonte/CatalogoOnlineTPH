...
// Apertura del Lightbox Modal
        ...
        function incrementInteractionCount() {
            if (sessionStorage.getItem(FEEDBACK_SHOWN_KEY) === "true") return;

            let count = parseInt(sessionStorage.getItem(FEEDBACK_INTERACTIONS_KEY) || "0", 10);
            count++;
            sessionStorage.setItem(FEEDBACK_INTERACTIONS_KEY, count.toString());
...
/* ========================================================
        ...
        function showFeedbackModal() {
            ...
            if (facebookUrl) {
                variants.push({
                    ...
                    buttonText: 'Visitar en Facebook',
                    url: facebookUrl,
                    icon: `<svg viewBox="0 0 24 24"><path d="M14 22V13h3l.5-4H14V7c0-1 .3-1.5 1.5-1.5h2V2c-.6 0-1.8-.1-3.2-.1C11.5 1.9 9 3.7 9 7.2V9H6v4h3v9h5z"/></svg>`
                });
            }
        ...
        function setupGoogleStars(reviewsUrl) {
            ...
            const stars = starsContainer.querySelectorAll('.feedback-star');
            stars.forEach(star => {
                star.addEventListener('mouseover', () => highlightStars(stars, parseInt(star.getAttribute('data-rating'), 10)));
                star.addEventListener('mouseout', () => highlightStars(stars, 0));
                star.addEventListener('click', () => {
                    const rating = parseInt(star.getAttribute('data-rating'), 10);
                    trackEvent(rating >= 4 ? 'valoracion_estrellas_alta' : 'valoracion_estrellas_baja', { estrellas: rating });
        ...
        function highlightStars(stars, rating) {
            stars.forEach(star => {
                star.classList.toggle('hovered', parseInt(star.getAttribute('data-rating'), 10) <= rating);
            });
        }
        ...
        window.handleFeedbackSubmit = function(isGoogle, type = '') {
            sessionStorage.setItem(FEEDBACK_SHOWN_KEY, "true");
            trackEvent(isGoogle ? 'conversion_feedback_google' : `conversion_feedback_${type}`, { accion: 'clic_enlace' });
